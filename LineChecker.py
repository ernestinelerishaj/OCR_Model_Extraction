...
def flatten_list(nested_list):
    ...
    for item in nested_list:
        if isinstance(item, list):
            result_str += flatten_list(item)  # Recursively flatten the nested lists
        else:
            ...
            if item.endswith(".00"):
                item=item[:-3]  
            print(item)
            if item.replace(",", "").isdigit():  # Check if it's a numeric string
                item = str(int(item.replace(",", "")))  # Convert to integer and back to string
            result_str += item
    return result_str
...
def main():
    ...
    # Path to your image
    image_path = "temp_image.jpg"  # Updated path to the image you uploaded
    print(LineWords(image_path,"total"))
main()